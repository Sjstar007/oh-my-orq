...
```bash
node .oh-my-orq/memory/cortex.js start-session "Implement: <brief prompt summary>"
...
node .oh-my-orq/memory/cortex.js save "Backend: <key backend implementation decisions>" --type architecture
```
...
```bash
node .oh-my-orq/memory/cortex.js save "Implementation complete: <one-line summary>" --type decision
